...
public abstract class ForwardingCache<K, V> implements Cache<K, V> {
    ...
    @Override public V get(K key, Function<? super K, ? extends V> loader) { return delegate().get(key, loader); }
    @Override public boolean contains(K key) { return delegate().contains(key); }
    ...
    @Override public void put(K key, Function<? super K, ? extends V> loader) { delegate().put(key, loader); }
    ...
    @Override public Map<K, V> getAllPresent(Iterable<? extends K> keys) { return delegate().getAllPresent(keys); }
    @Override public Map<K, V> asMap() { return delegate().asMap(); }
    @Override public Set<RemovalListener<K, V>> removalListeners() { return delegate().removalListeners(); }
